...
> 当前为 **0.1.1，已发布到 npm**。D1–D8 已交付源码/产物校验、只读归档 inspect、CLI、Vite Node alias、握手 bootstrap、浏览器类型适配、可复现且不覆盖已有文件的 `.aplg` pack、无副作用的 `vanilla-ts` init 模板，以及显式内存测试宿主和仅 loopback 的开发预览。D9 协调 CI 与受控发布入口已准备；0.1.0 已用一次性 granular token 完成首发，trusted publisher 已配置。
...
对外打包应使用 `pnpm pack` 将 `workspace:0.1.1` 转换成真实 `0.1.1`；不要用 npm pack 直接分发带 workspace: 的源码 package.json。两包 0.1.1 已发布到 npm，并已用仓库外消费者完成 install/build/pack/browser 验收。
