...
- 日期：2026-09-13
- 状态：总体方向、npm 包名和公开 PR 发布入口已确认；本文同步 `ai-switch/plugin-store` 的职责与信任边界，并已拆分 runtime/devkit 两份待执行计划。已创建远程仓库及贡献文档；runtime R1–R7 的协议、RPC、客户端/宿主、Node 纯 JS 子集与异步文件客户端已实施并本地验证，后续运行时、devkit 和自动发布流水线尚未实施，所有 npm 包均未发布。
