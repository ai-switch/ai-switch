...
- 日期：2026-09-13
- 状态：总体方向、npm 包名和公开 PR 发布入口已确认；本文同步 `ai-switch/plugin-store` 的职责与信任边界，并已拆分 runtime/devkit 两份实施计划。runtime R1–R8 与 devkit D1–D9 已在 Windows Node 22.22.2 与 Linux Node 22/24 本地实现并通过测试；协调 CI 与受控发布入口已准备；两包 0.1.0 已发布到 npm，trusted publisher 已配置为 GitHub Actions + `tauri-plugin-runtime.yml` + environment `aplg-npm-release`。真实 Rust/Tauri/Web 宿主接入、plugin-store 审核构建/签名/索引和原生扩展 profile 尚未实施。
...
| 原始设计交付与后续实施 | 原始阶段修订设计并创建 plugin-store；后续 runtime R1–R8 与 devkit D1–D9 已本地实现并通过 Windows/Linux Node 22/24 验收；两包 0.1.0 已发布到 npm 并配置 trusted publisher，插件 Release 发布未执行 |
...
本规格是架构总纲，不作为一个巨大任务一次实现。2026-09-14 已按用户要求完成前两份公共 npm 包的独立实施计划；runtime R1–R7 已实施，其他任务仍待实施：

- [runtime 实施计划](../plans/2026-09-14-tauri-plugin-runtime.md)：R1–R7 已完成，其余待实施；R1–R8 覆盖唯一协议/schema、宿主与插件入口、生命周期、Node 客户端及外部 tarball 验收。
