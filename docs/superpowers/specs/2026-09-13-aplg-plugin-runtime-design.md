...
- 日期：2026-09-13
- 状态：总体方向、npm 包名和公开 PR 发布入口已确认；本文同步 `ai-switch/plugin-store` 的职责与信任边界，并已拆分 runtime/devkit 两份待执行计划。已创建远程仓库及贡献文档；runtime R1/R2 的协议基础已实施并本地验证，后续运行时、devkit 和自动发布流水线尚未实施，所有 npm 包均未发布。
...
本规格是架构总纲，不作为一个巨大任务一次实现。2026-09-14 已按用户要求完成前两份公共 npm 包的独立实施计划；runtime R1/R2 已实施，其他任务仍待实施：

- [runtime 实施计划](../plans/2026-09-14-tauri-plugin-runtime.md)：R1/R2 已完成，其余待实施；R1–R8 覆盖唯一协议/schema、宿主与插件入口、生命周期、Node 客户端及外部 tarball 验收。
