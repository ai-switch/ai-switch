...
- 日期：2026-09-13
- 状态：总体方向、npm 包名和公开 PR 发布入口已确认；本文同步 `ai-switch/plugin-store` 的职责与信任边界，并已拆分 runtime/devkit 两份实施计划。已创建远程仓库及贡献文档；runtime R1–R8 的协议、桥、Node 客户端、独立 npm tarball 与无框架示例已实现，Windows Node/Chromium/WebKit 验收通过，Linux 因本机 WSL VHD 缺失待补验。devkit、真实 Rust/Tauri/Web 宿主接入与自动发布流水线尚未实施，所有 npm 包均未发布。
