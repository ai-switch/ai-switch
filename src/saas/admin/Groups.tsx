...
export function GroupsPanel() {
  ...
  async function resync(groupId: string) {
    ...
    setSyncError(null);
    try {
      await adminCall("groups.sync", { groupId });
      groups.reload();
    } catch (error) {
  ...
  return <>
    ...
    {groups.loading ? <Loading /> : groups.error ? <ErrorState error={groups.error} retry={groups.reload} /> : groups.data && <>
      {groups.data.items.length ? <div className="saas-group-grid">{groups.data.items.map(group => <Card key={group.id} title={<span className="saas-row"><Layers3 size={19} />{group.name}</span>}>
        ...
        {group.lastSyncError && <Notice tone="warning">{text("上次同步失败：", "Last sync failed: ")}{group.lastSyncError}</Notice>}
        <div className="saas-actions"><Button onClick={() => void resync(group.id)} busy={syncingId === group.id}><RefreshCw size={15} />{text("重新同步", "Resync")}</Button><Button onClick={() => setEditor(group)}><Pencil size={15} />{text("编辑 SaaS 配置", "Edit SaaS settings")}</Button></div>
...
type PriceDraft = { model: string; upstreamModel: string; input: string; cache: string; output: string; image: string; enabled: boolean; syncState?: "active" | "pending_pricing" | "stale"; managedByPool?: boolean; lastSeenAt?: number | null };

function SyncStateBadge({ state, enabled, text }: { state?: string; enabled?: boolean; text: (zh: string, en: string) => string }) {
  if (state === "pending_pricing") return <span className="saas-badge">{text("待定价", "Pending pricing")}</span>;
  if (state === "stale") return <span className="saas-badge">{text("已失效", "Stale")}</span>;
  if (enabled === false) return <span className="saas-badge">{text("已停用", "Disabled")}</span>;
  return <span className="saas-badge">{text("已启用", "Active")}</span>;
}
...
function GroupEditor({ existing, onClose, onSaved }: { existing: SaasGroup; onClose: () => void; onSaved: () => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    void action.run(() => {
      ...
      return adminCall("groups.save", { id:existing.id,multiplierMicros:decimalToInteger(multiplier),maxOutputTokens,timeoutSeconds,maxConcurrency,allowSubscription,allowBalance,
        models:prices.map(price => ({ model:price.model.trim(),upstreamModel:price.upstreamModel,inputPriceMicros:decimalToInteger(price.input),cachePriceMicros:decimalToInteger(price.cache),outputPriceMicros:decimalToInteger(price.output),imagePriceMicros:decimalToInteger(price.image || "0"),enabled:price.enabled,syncState:price.syncState,managedByPool:price.managedByPool,lastSeenAt:price.lastSeenAt })) });
    },onSaved);
  }
  return <Dialog title={existing.configured ? text("编辑 SaaS 配置", "Edit SaaS settings") : text("添加 SaaS 分组", "Add SaaS group")} busy={action.busy} onClose={onClose}><form className="saas-form" onSubmit={submit}>
    ...
    <fieldset><legend>{text("允许的模型", "Allowed models")}</legend>{modelChoices.map(model => <CheckField key={model} label={model} checked={prices.some(price => price.upstreamModel===model)} onChange={checked => setPrices(current => checked ? [...current,{model,upstreamModel:model,input:"",cache:"",output:"",image:"",enabled:true,syncState:"active"}] : current.filter(price => price.upstreamModel!==model))} />)}</fieldset>
    {prices.map((price,index) => <div className="saas-form-grid" key={`${price.upstreamModel}:${index}`}>
      {existing.platform === "claude"
        ? <Field label={text(`公开模型名 ${index+1}`, `Public model name ${index+1}`)}><input readOnly value={price.upstreamModel} /></Field>
      ...
      <Field label={text(`状态 ${index+1}`, `Status ${index+1}`)}><SyncStateBadge state={price.syncState} enabled={price.enabled} text={text} /></Field>
      ...
      <div className="saas-row"><CheckField label={text(`启用 ${index+1}`, `Enabled ${index+1}`)} checked={price.enabled} onChange={enabled => setPrices(current => current.map((entry,offset) => offset===index ? {...entry,enabled,syncState:enabled && entry.syncState === "pending_pricing" ? "active" : entry.syncState}:entry))} /></div>
    </div>)}
