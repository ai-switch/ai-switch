...
Claude 系列的 1M 上下文要靠 `anthropic-beta: context-1m-2025-08-07` 声明，而不同上游对「没声明」的反应不一样：有的按 200K 截断，有的直接拒绝并回一句 `1m 上下文已经全量可用，请启用 1m 上下文后重试`。代理在下面任一条成立时就补上这个头：

- 客户端自己在模型名上带了 `[1M]` 后缀（Claude Code 的写法）。匹配映射时会剥掉后缀，所以这个意图只能从改写前的原始请求体里读。
- **服务该请求的账号**，其映射勾了「1M」（`supports_1m`）。这一条刻意按账号判定：写进客户端配置的 `[1M]` 后缀是池级的，池里只要有一个账号没勾，整个池的该角色都会被省略后缀，于是能服务 1M 的账号反而收到一个没有声明的请求。
- Codex 平台，且该映射的上游模型被声明或推算为 1M（Codex 不需要模型名变体）。

勾了「1M」的映射还会在模型列表里额外暴露一个 `别名[1m]` 变体。
...
清单形态由每个平台自己的「模型清单模式」决定，在「接入算力池」弹窗顶部切换，默认聚合：

| 模式 | 清单形态 | 选号行为 |
| --- | --- | --- |
| 聚合模式 | `gpt-5.6-sol` | 池内轮换 |
| 精确模式 | `Grox/gpt-5.6-sol`、`official/gpt-5.6-sol` | 前者钉死在名为 Grox 的 API 账号；后者在全部官方账号之间轮换 |

精确模式只展开 **API 类型**账号。官方账号合并成一份挂在保留前缀 `official/` 下：它们的请求体不套模型映射，能服务的模型集合完全相同，而池化官方账号的意义就是配额轮换，钉住一个正好把它抵消掉。

前缀由账号名安全化而来——空格、`/`、`:` 一律换成 `-`，中文保留，截到 32 字符；两个账号安全化后同名时，两边都会补上账号 id 前 6 位（如 `TaBiAI-a1b2c3`）。改了账号名要重新写入客户端配置，旧名字不再解析。

三点行为值得记住：

- 代理在**任何模式下都接受**带前缀的模型名，所以切回聚合模式不会让已写出去的配置失效，在客户端里手打 `账号/模型` 也能用。
- 前缀解不出账号、或该账号不支持余下的别名时，整串会当作模型名继续走——中转站常见的 `z-ai/glm-5.3` 这类带厂商路径的模型名不受影响。
- Claude Code 的四个 `/model` 槽位不受影响：槽位写的是单个别名而不是清单，它继续走轮换。gemini 原生路径把模型放在 URL 里，不解析前缀。

### 客户端模型名：alias 还是真实上游模型

同一份映射在两个地方有不同名字，取决于谁在看它：

- **Claude Code 官方配置**使用 alias 槽位（如 `claude-sonnet-alias`）：这是 Claude Code 的客户端限制，槽位只能填固定别名，代理再按账号把它改写成 `to`。
- **第三方客户端与 Claude SaaS** 使用真实上游 `to` 模型名：ZCode、WorkBuddy/CodeBuddy、Qoder CLI、DeepSeek Harness 的模型清单，以及 SaaS 的 `/v1/models`、计费和账号筛选，对外都是 `to`，不再泄露 Claude Code 的 alias。

Claude SaaS 是计费产品目录，**始终使用裸 `to`，不暴露账号前缀**：同一真实模型只有一份价格，账号改名或增删不会改变用户看到的模型 ID。账号前缀只用于第三方客户端的精确模式（`{账号前缀}/{to}`）。

自动同步发现的模型先进入「待定价」状态：**未定价或已失效的模型不会出现在 SaaS `/v1/models`，也不会创建计费预留**，必须由管理员显式定价并启用后才会对外开放。模型暂时离开算力池时会转为「已失效」并保留历史价格，重新出现后自动恢复；管理员也可以在管理端删除已失效行的历史价格，该操作只清理 SaaS 价格记录，不影响算力池。
...
CLI 请求
  ...
  ├─ 模型名形如「账号/别名」且前缀能解出账号？→ 候选只留该账号，模型名换成裸别名
