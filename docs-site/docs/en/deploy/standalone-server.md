...
Since v0.8.6, the Linux standalone server binary does not require WebKitGTK. The installer still checks for missing libraries with `ldd`; if you pin an older version that lacks WebKitGTK, Debian/Ubuntu installs `libwebkit2gtk-4.1-0` automatically, while other distributions require the equivalent runtime package first.
