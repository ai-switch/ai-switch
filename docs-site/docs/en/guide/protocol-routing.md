...
- **`openai` / `openai-responses`**: applies Codex CLI's client identity for the same fingerprinting reason; the request path has its leading version segment stripped (`/v1/...` → `/...`) before the base URL re-attaches it. The identity is swapped as a whole rather than topped up field by field: a caller that is itself an official Codex client (its `user-agent` belongs to the official client family and carries a parseable `X.Y.Z` engine version) keeps its own, and only gets the matching `originator` filled in; anything else has its identity replaced outright, along with dropping the Anthropic SDK headers it brought — half a Codex identity is rejected more readily than either whole one. An `x-codex-window-id` engine fingerprint is added too, unless the caller already sent some `x-codex-*` header: a relay's "official Codex clients only" restriction requires one by default, and without it every turn on that account comes back as `This account only allows Codex official clients`. When the account pins a `User-Agent` of its own, the whole impersonation steps aside rather than building half an identity around it.
