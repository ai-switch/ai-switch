---
title: 常见问题
description: AI Switch 常见问题解答：与手改配置文件的区别、平台支持范围、协议桥接、19527 共享监听端口的面板与模型流量差异、密钥存储安全性、额度切换、手机访问、数据备份与开源许可。
---
...
路由接入是独立开关：关闭它不会停止共享端口，Web 页面与健康检查仍可用；开启它时如果端口未启动，会自动启动共享端口。路由代理的配置见[账号与算力池](/guide/accounts)，Web 服务的配置见[Web 服务模式](/deploy/web-service)。
...
- 默认绑定 `127.0.0.1:19527`，只有本机能访问。手机要访问必须改绑定地址或走 Tailscale。
- 桌面设置可选择 `0.0.0.0`，无需 TLS 即可向局域网开放；全部 Web 命令都会通过明文 HTTP 提供，访问令牌是唯一保护，请只在可信网络使用。独立 server 的非回环明文限制不变。
...
- 仅行尾差异导致的校验和不匹配会被**原地修复**，不再隔离数据库；
- 如果本地库是空的、而 `backups/` 里存在被隔离的库，启动时会**自动恢复**它（先在副本上验证能升级到当前结构，且绝不覆盖你在此之后新建的账号）；
- 真正改动过迁移内容且库里有数据时，应用会**报错拒绝启动**，而不是替换数据库。
